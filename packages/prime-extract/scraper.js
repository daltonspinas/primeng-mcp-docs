...
function extractApiSection($, apiSection) {
  ...
  $(apiSection)
    .find("app-docsection section")
    .each((i, section) => {
      const sectionTitle = $(section)
        .find("h2.doc-section-label")
        .text()
        .replace("#", "")
        .trim();
      const sectionDesc = $(section)
        .find(".doc-section-description p")
        .first()
        .text()
        .trim();
      ...
      $(section)
        .find("app-docapitable")
        .each((j, apiTable) => {
          const subTitle = $(apiTable)
            .find("h3.doc-section-label")
            .text()
            .replace("#", "")
            .trim();
          const subDesc = $(apiTable)
            .find(".doc-section-description p")
            .text()
            .trim();
...
function extractThemingSection($, themingSection) {
  ...
  $(themingSection)
    .find("app-docapitable")
    .each((i, apiTable) => {
      const sectionTitle = $(apiTable)
        .find("h3.doc-section-label")
        .text()
        .replace("#", "")
        .trim();
      const sectionDesc = $(apiTable)
        .find(".doc-section-description p")
        .text()
        .trim();
  ...
  if (presetsSection.length > 0) {
    const presetsTitle = presetsSection
      .find("h3.doc-section-label")
      .text()
      .replace("#", "")
      .trim();
    const presetsDesc = presetsSection
      .find(".doc-section-description p")
      .first()
      .text()
      .trim();
...
async function scrapeComponents() {
  ...
  for (const comp of componentList) {
    ...
    try {
      ...
      if (!docMain.length) {
        console.log(
          `No doc-main found for ${comp.name}, trying alternative selectors...`
        );
        continue;
      }
      ...
      if (docSections.length > 0) {
        ...
        docSections.each((i, section) => {
          ...
          if (sectionData.title) {
            ...
            const anchorId = sectionData.title
              .toLowerCase()
              .replace(/[^a-z0-9]+/g, "-")
              .replace(/^-|-$/g, "");
        ...
        if (apiData.title) {
          ...
          apiData.sections.forEach((section) => {
            ...
            section.subsections.forEach((subsection) => {
              if (subsection.title) {
                const anchorId = subsection.title
                  .toLowerCase()
                  .replace(/[^a-z0-9]+/g, "-")
                  .replace(/^-|-$/g, "");
