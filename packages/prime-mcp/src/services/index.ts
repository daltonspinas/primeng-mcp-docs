...
class PrimeNGMCPServer {
        ...
        tools: [
          {
            ...
            description:
              "**ALWAYS CALL THIS FIRST** - Shows all available PrimeNG components with descriptions. Returns the exact component names needed for get_primeng_component_docs. This eliminates guessing and ensures you use valid component names.",
            inputSchema: {
              type: "object",
              properties: {
                filter: {
                  type: "string",
                  description:
                    "Optional filter to search components by name or functionality. Leave empty to see all components.",
                },
              },
            },
          },
          {
            ...
            description:
              "Get complete documentation for a PrimeNG component. **Must use exact component name from list_all_primeng_components.** This will fail if you guess the component name or use incorrect capitalization.",
            inputSchema: {
              type: "object",
              properties: {
                component: {
                  type: "string",
                  description:
                    "Exact component name from list_all_primeng_components (case-sensitive). Copy directly from the list to ensure accuracy.",
                },
              },
              required: ["component"],
            },
      ...
      try {
        switch (name) {
          case "list_all_primeng_components":
            return await this.handleListAllComponents(args?.filter as string);
  ...
  private async handleListAllComponents(filter?: string) {
...
**Or use a broader search term** like "input", "table", "button", "dialog", etc.`
    ...
    const componentList = results
      .map(
        (comp) =>
          `**${comp.name}** - ${comp.description || "PrimeNG component"}`
      )
      .join("\n");
...
2. \`get_primeng_component_docs("Button")\` → Get Button documentation`,
    ...
    try {
      ...
      const allComponents = await this.documentationService.getAllComponents();
      const exactMatches = allComponents.filter(
